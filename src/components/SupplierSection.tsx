  ...
  return (
    <section className="mt-4 rounded-xl bg-surface p-4 shadow-sm">
      <div className="mb-2 flex items-center justify-between">
        <h2 className="flex items-center gap-1.5 text-sm font-semibold text-body">
          <Icon name="truck" className="h-4 w-4 text-faint" /> Supplier progress
          <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-700">BOSS</span>
        </h2>
        <div className="flex items-center gap-3">
          <Link to="/suppliers" className="text-xs font-medium text-muted-2 underline">
            Manage
          </Link>
          <button
      ...
      {suppliers.length > 0 && (
        <div className="mb-3 space-y-2">
          ...
          <div className="flex justify-between text-xs text-muted-2">
            ...
            <span className="font-semibold tabular-nums text-body">{money(total)}</span>
          </div>
        </div>
      )}
      ...
      {adding && (
        <div className="mb-3 space-y-2 rounded-lg border border-line p-3">
          ...
          <button
            ...
            onClick={addRow}
            className="w-full rounded-lg bg-primary py-2 text-sm font-medium text-white active:bg-primary-press disabled:opacity-60"
          >
            Add supplier
          </button>
        </div>
      ...
      {suppliers.length === 0 && !adding ? (
        <p className="py-6 text-center text-sm text-faint">No suppliers tracked yet.</p>
      ) : (
            ...
            return (
              <div key={cat || 'untagged'}>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  {sub > 0 && <span className="text-xs tabular-nums text-faint">{money(sub)}</span>}
                </div>
    ...
    <li className="rounded-xl border border-line p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate font-semibold text-ink">{s.supplier || '—'}</p>
          {s.item && <p className="truncate text-xs text-muted-2">{s.item}</p>}
        </div>
        <div className="shrink-0 text-right">
          {Number(s.cost) > 0 && <p className="text-sm font-semibold tabular-nums text-ink-2">{money(Number(s.cost))}</p>}
          {s.expected_date && <p className="text-xs text-faint">{formatDate(s.expected_date)}</p>}
        </div>
      </div>

      <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-fill dark:bg-slate-700">
      ...
      {editing ? (
        <div className="mt-2 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-lg border border-line-2 px-3 py-1.5 text-xs font-medium text-muted active:bg-press"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <button onClick={() => setEditing(true)} className="min-w-0 flex-1 truncate text-left text-xs text-muted-2">
            {s.notes ? s.notes : <span className="text-faint">+ Add notes</span>}
          </button>
          <select
            value={s.category || ''}
            disabled={busy}
            ...
            title="Move to trade"
            className="shrink-0 rounded-lg border border-line-2 px-1.5 py-1 text-[11px] text-muted"
          >
