...
import { calcCosting, money, num } from '../lib/costing'

type CostRow = { label: string; amount: string }
type PctRow = { name: string; percent: string }
...
export default function CostingForm() {
  ...
  const [commissions, setCommissions] = useState<PctRow[]>([])
  const [shares, setShares] = useState<PctRow[]>([])
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        if (c) {
          ...
          setCommissions((c.commissions ?? []).map((x) => ({ name: x.name, percent: String(x.percent) })))
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const row = {
      ...
      commissions: commissions.filter((c) => c.name.trim()).map((c) => ({ name: c.name.trim(), percent: num(c.percent) })),
      shares: shares.filter((c) => c.name.trim()).map((c) => ({ name: c.name.trim(), percent: num(c.percent) })),
      notes: notes.trim(),
      status,
    }
    const res = isEdit
  ...
  return (
    <Layout title={isEdit ? 'Edit costing' : 'New costing'} back={<BackLink />}>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="space-y-4 rounded-xl bg-white p-4 shadow-sm">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={field} value={cashSaleNo} onChange={(e) => setCashSaleNo(e.target.value)} placeholder="e.g. CS-1042" />
            </div>
            <div>
          ...
          {costs.map((c, i) => (
            <div key={i} className="flex gap-2">
              <input
                ...
                value={c.label}
                onChange={(e) => setCosts(upd(costs, i, { label: e.target.value }))}
              />
              <input
                type="number" step="0.01" className={field + ' w-28'} placeholder="RM"
                value={c.amount}
                onChange={(e) => setCosts(upd(costs, i, { amount: e.target.value }))}
              />
        ...
        {/* Net profit */}
        <Figure label="Net profit (revenue − costs)" value={calc.netProfit} strong />

        {/* Commissions */}
        <Section title="Commissions (% of net profit)" total={money(calc.totalCommission)}>
          {commissions.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              <input
                className={field + ' flex-1'} list="people" placeholder="Staff name"
                value={c.name}
                onChange={(e) => setCommissions(upd(commissions, i, { name: e.target.value }))}
              />
              <div className="flex items-center gap-1">
                <input
                  type="number" step="0.1" className={field + ' w-20'} placeholder="%"
                  value={c.percent}
                  onChange={(e) => setCommissions(upd(commissions, i, { percent: e.target.value }))}
                />
          ...
          <AddBtn label="+ Add commission" onClick={() => setCommissions([...commissions, { name: '', percent: '' }])} />
        </Section>

        {/* Profit after commission */}
        <Figure label="Profit after commission" value={calc.profitAfterComm} strong />

        {/* Profit sharing */}
        <Section title="Profit sharing (% of profit after commission)" total={money(calc.totalShared)}>
          {shares.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              <input
                className={field + ' flex-1'} list="people" placeholder="Partner name"
                value={c.name}
                onChange={(e) => setShares(upd(shares, i, { name: e.target.value }))}
              />
              <div className="flex items-center gap-1">
                <input
                  type="number" step="0.1" className={field + ' w-20'} placeholder="%"
                  value={c.percent}
                  onChange={(e) => setShares(upd(shares, i, { percent: e.target.value }))}
                />
                <span className="text-slate-400">%</span>
              </div>
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm">
          ...
          <label className="mt-3 flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" checked={status === 'finalized'} onChange={(e) => setStatus(e.target.checked ? 'finalized' : 'draft')} />
            Mark as finalized
          </label>
        </div>
      ...
      {isEdit && (
        <button
          onClick={remove}
          disabled={busy}
          className="my-5 w-full rounded-lg border border-red-200 py-2.5 text-sm font-medium text-red-600 active:bg-red-50 disabled:opacity-60"
        >
          Delete costing
        </button>
      )}
...
function Figure({ label, value, strong }: { label: string; value: number; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-xl bg-slate-900 px-4 py-3 text-white">
      ...
      <span className={(strong ? 'text-lg font-bold ' : 'font-medium ') + (value < 0 ? 'text-rose-300' : '')}>
        {money(value)}
      </span>
    </div>
  )
...
function AddBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="text-sm font-medium text-slate-500 active:text-slate-800">
      {label}
    </button>
  )
}
...
function RemoveBtn({ onClick }: { onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="shrink-0 rounded-lg bg-rose-50 px-2 text-rose-500 active:bg-rose-100" title="Remove">
      ✕
    </button>
  )
}
...
function BackLink() {
  return (
    <Link to="/costing" className="text-xl leading-none text-slate-300">
      ←
    </Link>
  )
}
